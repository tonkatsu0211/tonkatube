      ...
      try {
        ...
        stream.on("error", (err) => {
          ...
          if (retryCount < MINIGET_RETRY_LIMIT) {
            console.log(
              `Retrying... (${retryCount + 1}/${MINIGET_RETRY_LIMIT})`
            );
            setTimeout(
              () => fetchStreamWithRetry(url, retryCount + 1),
              RETRY_DELAY_MS
            );
          } else {
        ...
        if (retryCount < MINIGET_RETRY_LIMIT) {
          console.log(
            `Error fetching stream. Retrying... (${
              retryCount + 1
            }/${MINIGET_RETRY_LIMIT})`
          );
          setTimeout(
          ...
          res
            .status(500)
            .send("Failed to fetch stream after multiple retries.");
        }
      }
    };
